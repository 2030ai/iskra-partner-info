...
for required_command in find git npx rg; do
...
if [ "${#PUBLIC_MD_FILES[@]}" -gt 0 ]; then
  blocking_rg -n "2030AI|/(Users|home|sessions)/|\bTBD\b|\bTODO\b|PLACEHOLDER|\[ответственный\]" "${PUBLIC_MD_FILES[@]}"
fi

section "secret-like public scan"
if [ "${#PUBLIC_MD_FILES[@]}" -gt 0 ]; then
  blocking_rg -ni "password|token|secret|ssh|private key|DemoPass|api key|bank|iban|бик|расч[её]тный сч[её]т|корреспондентский сч[её]т" "${PUBLIC_MD_FILES[@]}"
fi

section "hard risky wording scan"
if [ "${#PUBLIC_MD_FILES[@]}" -gt 0 ]; then
  blocking_rg -ni "данные никогда|не уходят наружу|не передаются третьим лицам|не покидают периметр|полностью соответствует 152-фз|сертифицирован[а-я ]*фстэк" "${PUBLIC_MD_FILES[@]}"
fi

section "hard unconfirmed public claim scan"
if [ "${#CLAIM_SCAN_FILES[@]}" -gt 0 ]; then
  blocking_rg -ni "без внешних LLM|полностью локально|включ[её]н[аоы]?[[:space:]]+в[[:space:]]+реестр российского ПО|внес[её]н[аоы]?[[:space:]]+в[[:space:]]+реестр российского ПО|в реестре российского ПО" "${CLAIM_SCAN_FILES[@]}"
fi

section "partner economics public scan"
if [ "${#PUBLIC_MD_FILES[@]}" -gt 0 ]; then
  blocking_rg -ni "вознагражд|рефераль|реферал|партн[её]рск[а-я ]*(процент|выплат|комисс|вознагражд|доход|заработ)|получайте [0-9]+%|зарабатывайте|[0-9]+%.*(платеж|оплат|привед[её]н|клиент|партн[её]р|комисс|вознагражд)|(промо-код|промокод).*(экономик|вознагражд|выплат|комисс|процент|%)" "${PUBLIC_MD_FILES[@]}"
fi

section "agent hard leak scan"
AGENT_LEAK_FILES=("${AGENT_DOC_FILES[@]}" "${AGENT_SCRIPT_FILES[@]}")
if [ "${#AGENT_DOC_FILES[@]}" -gt 0 ]; then
  blocking_rg -n "\bTBD\b|\bTODO\b|PLACEHOLDER|\[ответственный\]" "${AGENT_DOC_FILES[@]}"
fi
if [ "${#AGENT_LEAK_FILES[@]}" -gt 0 ]; then
  blocking_rg -n "/(Users|home|sessions)/[A-Za-z0-9._/-]+" "${AGENT_LEAK_FILES[@]}"
  blocking_rg -ni "BEGIN [A-Z ]*PRIVATE KEY|ssh-rsa [A-Za-z0-9+/=]{40,}|(DemoPass|api[ _-]?key|password|token|secret|iban|бик|расч[её]тный сч[её]т|корреспондентский сч[её]т)\s*[:=]" "${AGENT_LEAK_FILES[@]}"
fi
